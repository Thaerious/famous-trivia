...
class EJSRender {
    ...
    doBrowserify(jsFilePath, htmlFilePath){
        ...
        if (htmlFilePath){
            ...
            for (let dep of dependencies) {
                let path = getScriptPath(constants.NIDJET_SCRIPT_PATH, dep);
                b.add(path);
            }
        }
    ...
    static async render(nidgetPreprocessor){
        let files = fs.readdirSync(config.EJS_ROOT_DIR);
        let jit = new EJSRender(nidgetPreprocessor);

        if (!fs.existsSync(config.GENERATED_SCRIPT_DIR)){
            fs.mkdirSync(config.GENERATED_SCRIPT_DIR, {recursive : true});
        }

        if (!fs.existsSync(config.GENERATED_EJS_DIR)){
            fs.mkdirSync(config.GENERATED_EJS_DIR, {recursive : true});
        }
    ...
    static async renderEJS(filename, nidgetDependencies){
        ejs.renderFile(
            Path.join(config.EJS_ROOT_DIR, filename),
            {
            ...
            (err, str)=>{
                ...
                fs.writeFileSync(Path.join(config.GENERATED_EJS_DIR, filename.slice(0, -4) + ".ejs"), str);
            }
        );
    }
    ...
    static async renderJS(jit, filename, nidgetPreprocessor){
        ...
        const jsFilePath = config.client_path + name + ".js";
        const htmlFilePath = config.EJS_ROOT_DIR + name + ".ejs";
        const outfile = Path.join(config.GENERATED_SCRIPT_DIR, name + ".js");
