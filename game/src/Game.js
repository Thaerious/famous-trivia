...
class Game {
    ...
    startRound () {
        const update = this.gameModel.getUpdate();
    ...
    [4] (input) {
        // waiting for player to pick question
        switch (input.action) {
        case `select`: {
            let allow = false;
    ...
    [5] (input) {
        // waiting for host to read question and click continue
        switch (input.action) {
        case `continue`:
            if (input.player !== config.names.HOST) return;
            this.updateState(6);
    ...
    [6] (input) {
        // timer not expired awaiting answer
        switch (input.action) {
        case `reject`:
            if (!this.gameModel.getRound().isLowestUnspent()) {
                const currentPlayer = this.gameModel.getRound().getCurrentPlayer();
                const player = this.gameModel.getPlayer(currentPlayer);
    ...
    [8] (input) {
        // awaiting answer to jeopardy question
        switch (input.action) {
        case `reject`: {
            const currentPlayer = this.gameModel.getRound().getCurrentPlayer();
            const player = this.gameModel.getPlayer(currentPlayer);
...
export { Game, GameModel, Timer, constants };
