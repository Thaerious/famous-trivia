...
/**
 * Renders EJS pages into HTML pages.
 */
class EJSRender {
    ...
    static async render(nidgetPreprocessor){
        let files = fs.readdirSync(config.server.EJS_ROOT_DIR);
        let jit = new EJSRender(nidgetPreprocessor);

        if (!fs.existsSync(config.server.GENERATED_SCRIPT_DIR)){
            fs.mkdirSync(config.server.GENERATED_SCRIPT_DIR, {recursive : true});
        }

        if (!fs.existsSync(config.server.GENERATED_EJS_DIR)){
            fs.mkdirSync(config.server.GENERATED_EJS_DIR, {recursive : true});
        }

        for (let filename of files) {
            console.log("Render: " + filename);
            let nidgetDependencies = nidgetPreprocessor.getTemplateDependencies(config.server.EJS_ROOT_DIR + filename);
    ...
    static async renderEJS(filename, nidgetDependencies){
        ejs.renderFile(
            Path.join(config.server.EJS_ROOT_DIR, filename),
            {
            ...
            (err, str)=>{
                ...
                fs.writeFileSync(Path.join(config.server.GENERATED_EJS_DIR, filename.slice(0, -4) + ".ejs"), str);
            }
        );
    }
    ...
    static async renderJS(jit, filename, nidgetPreprocessor){
        ...
        const htmlFilePath = config.server.EJS_ROOT_DIR + name + ".ejs";
        const outfile = Path.join(config.server.GENERATED_SCRIPT_DIR, name + ".js");
